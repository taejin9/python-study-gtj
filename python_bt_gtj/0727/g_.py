...
# 3. len() 함수
# 컨테이너에 있는 우너소의 개수를 반환
# 리스트, 튜플, 문자열, 딕셔너리 모든 형태 사용 가능
...
listt=[5,2,6,1,4]
print(sorted(listt))  #[1,2,4,5,6]

# 내림차순 정렬
print(sorted(listt,reverse=True))  #[6,5,4,2,1]

# 4. zip() 함수
# 여러 개의 순회 가능한 값(iterable)을 받아
# 각 객체의 동일한 인덱스에 있는 요소들끼리 짝지어 튜플로 반환

# 결괏값: 각각의 순회 가능한 값의 길이가 다를 경우
# 동일한 인덱스 번호의 값만을 추출(가장 짧은 길이의 값을 기준으로 튜플 반환)
